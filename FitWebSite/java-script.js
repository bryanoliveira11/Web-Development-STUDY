...
var itemCounter = 0 // counter that verify the cart emptyness
...
document.querySelector("#bmiCalc").onclick = () => {
    ...
    let IdealWRes = document.getElementById('idealW')
    let LossRes = document.getElementById('Loss')
    ...
    if (height === "" || isNaN(height))
        ...
        if (bmi < 18.6) {
            ...
            var idealWeight = (height - 100) * 0.87
            var loss = weight - idealWeight
            results.textContent = `${bmi} : Over Weight`
            IdealWRes.textContent = `Ideal Weight = ${idealWeight.toFixed(2)} kg`
            LossRes.textContent = `Weight Target : ${loss.toFixed(2)} kg`
        }
    }
}
